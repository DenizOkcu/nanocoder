---
name: plan:review
description: Comprehensive code review and quality assurance. Takes issue-name from /plan:implement. Runs linting, type checking, tests, security checks, and performs manual code review. Creates CODE_REVIEW.md with findings. Final step before deployment.
parameters:
	- issue-name
---
...
**IMPORTANT:** All planning artifacts are organized in `.nanocoder/planning/{{issue-name}}/` folders.

### Issue Name Parameter

**The issue name is: `{{issue-name}}`**

This was generated by `/plan:research` and used throughout the workflow.

Example usage:

```
/plan:review jwt-authentication
/plan:review fix-terminal-flickering
/plan:review user-dashboard
```

If no issue name is provided:

1. Look for directories in `.nanocoder/planning/`
...
First, locate and read the planning artifacts in `.nanocoder/planning/{{issue-name}}/` (if they exist):

- `.nanocoder/planning/{{issue-name}}/STATUS.md` - Current workflow status and progress
- `.nanocoder/planning/{{issue-name}}/IMPLEMENTATION_PLAN.md` - What was supposed to be built
- `.nanocoder/planning/{{issue-name}}/PROJECT_SPEC.md` - Technical requirements and design
- `.nanocoder/planning/{{issue-name}}/CODE_RESEARCH.md` - Original research and patterns
...
**Update `.nanocoder/planning/{{issue-name}}/STATUS.md`** when starting:

```markdown
## Progress

- [x] Research | [x] Design | [x] Implementation | [~] Review | [ ] Deploy
...
## Output: .nanocoder/planning/{{issue-name}}/CODE_REVIEW.md

Create a **focused** review report in `.nanocoder/planning/{{issue-name}}/CODE_REVIEW.md`:
...
- **Read `.nanocoder/planning/{{issue-name}}/STATUS.md` first** - understand the workflow context
...
- **Update `.nanocoder/planning/{{issue-name}}/STATUS.md`** - reflect review completion and results
- **Give credit** - note what was done well

### Update STATUS.md After Review

**Update `.nanocoder/planning/{{issue-name}}/STATUS.md`** based on outcome:
...
- [x] Research | [x] Design | [x] Implementation | [x] Review | [ ] Deploy
...
- **Critical:** [N issues]
- **Important:** [M issues]
- **Action:** Fix issues, re-run `/plan:review {{issue-name}}`
```
...
- Minor suggestions documented in `.nanocoder/planning/{{issue-name}}/CODE_REVIEW.md`

**You can now:**
1. Commit the changes: `git add . && git commit -m "feat: [description]"`
2. Push to remote: `git push`
3. Create a pull request or deploy
...
- Minor issues documented in `.nanocoder/planning/{{issue-name}}/CODE_REVIEW.md`
- Safe to deploy with follow-up tasks

You can proceed with:
1. Deploy now and address notes later, or
2. Fix the noted issues first, then re-run `/plan:review {{issue-name}}`
...
See `.nanocoder/planning/{{issue-name}}/CODE_REVIEW.md` for detailed findings and recommendations.

After fixing these issues, run:
/plan:review {{issue-name}}
...
2. Fix the issues
3. Run `/plan:review {{issue-name}}` again

See `.nanocoder/planning/{{issue-name}}/CODE_REVIEW.md` for detailed failure information.
```
