---
name: plan:research
description: Start a new feature workflow. Takes a feature description, generates an issue name, and performs deep codebase research. Creates CODE_RESEARCH.md with findings. First step of the planning workflow.
parameters:
  - feature-description
---
...
**IMPORTANT:** The parameter provided is the **feature description**, NOT the issue name.

Example usage:
```
/plan:research Add JWT authentication with refresh tokens
/plan:research Fix terminal flickering bug
/plan:research Implement user dashboard with analytics
```

## Issue Name Generation

**You MUST generate an issue name from the feature description:**

Feature description: {{feature-description}}

1. Extract a short, descriptive name from the feature description
2. Convert to kebab-case (lowercase with hyphens)
3. Keep it concise (2-5 words)

**After generating the issue name, inform the user:**
```
📋 Generated issue name: `{{issue-name}}`

All artifacts will be stored in the directory: `.nanocoder/planning/{{issue-name}}/`
```

## Your Mission

This is the **FIRST STEP** in the feature development workflow. You will:
1. Generate an issue name from the feature description
2. Conduct thorough research on the codebase
3. Create planning artifacts in `.nanocoder/planning/{{issue-name}}/`


## File Organization

**All planning artifacts must be organized in `.nanocoder/planning/{{issue-name}}/` folders.**

### Directory Setup

Before creating any artifacts:

1. Generate the issue name from the feature description
2. Create the directory: `.nanocoder/planning/{{issue-name}}/`
...
## Output: .nanocoder/planning/{{issue-name}}/CODE_RESEARCH.md

Create a **concise** research document in `.nanocoder/planning/{{issue-name}}/CODE_RESEARCH.md`:
...
Create `.nanocoder/planning/{{issue-name}}/STATUS.md`:
...
**Risk:** [Low/Medium/High] | **Started:** [timestamp]

## Progress

- [x] Research | [ ] Design | [ ] Implementation | [ ] Review | [ ] Deploy

## Phase: Research ✓

- **Key Findings:** [top 3, bullets]
- **Recommendations:** [top 3, bullets]
- **Next:** `/plan:design {{issue-name}}`
...
Research complete! The findings have been documented.

**Issue name:** `{{issue-name}}`
**Location:** `.nanocoder/planning/{{issue-name}}/`

**Recommended next command:**
/plan:design {{issue-name}}
