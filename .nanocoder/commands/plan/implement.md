---
name: plan:implement
description: Execute implementation from IMPLEMENTATION_PLAN.md and PROJECT_SPEC.md. Takes issue-name from /plan:design. Systematically implements code following documented phases, creates tests, and tracks progress. Third step of the planning workflow.
parameters:
	- issue-name
---
...
This was generated by `/plan:research` and used by `/plan:design` in previous steps.

Example usage:

```
/plan:implement jwt-authentication
/plan:implement fix-terminal-flickering
...
2. Use the most recent directory with `STATUS.md` showing "Design" complete or "Implementation" in progress
3. If multiple directories exist, ask the user which issue to work on

## Your Task

Execute the implementation plan and project specification created by the /plan:design command.

## Process

### 1. Read Planning Artifacts

First, locate and read the planning documents in `.nanocoder/planning/{{issue-name}}/`:

- `.nanocoder/planning/{{issue-name}}/IMPLEMENTATION_PLAN.md` - Your step-by-step guide
- `.nanocoder/planning/{{issue-name}}/PROJECT_SPEC.md` - Technical requirements and design
- `.nanocoder/planning/{{issue-name}}/STATUS.md` - Current workflow status

If IMPLEMENTATION_PLAN.md or PROJECT_SPEC.md don't exist, inform the user and suggest running `/plan:design {{issue-name}}` first.
...
- **Update `.nanocoder/planning/{{issue-name}}/STATUS.md`** with completion details

**Update `.nanocoder/planning/{{issue-name}}/STATUS.md`** when complete:
...
- **ALWAYS read `.nanocoder/planning/{{issue-name}}/IMPLEMENTATION_PLAN.md` and `PROJECT_SPEC.md` first**
...
1. Locate the issue directory in `.nanocoder/planning/{{issue-name}}/`
...
- Update the IMPLEMENTATION_PLAN.md and re-run /plan:implement {{issue-name}}
```
